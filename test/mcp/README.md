# Ritivel Development MCP Server

This directory contains a Model Context Protocol (MCP) server that provides Playwright browser automation capabilities for Ritivel development and testing. The MCP server exposes Ritivel's Playwright testing infrastructure through a standardized interface, allowing AI assistants and other tools to interact with browsers programmatically.
...
│   ├── playwright.ts        # Ritivel Playwright integration
...
- **Ritivel Integration**: Uses Ritivel's existing Playwright test infrastructure
...
### Server Won't Start
- Ensure Ritivel has been built and run at least once (via F5 or `code.sh`)
...
This MCP server is part of the Ritivel development infrastructure. When making changes:
