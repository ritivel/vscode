...
function code() {
	...
	if [[ "$OSTYPE" == "darwin"* ]]; then
		# Find the actual .app bundle in .build/electron directory
		APP_BUNDLE=$(find .build/electron -maxdepth 1 -name "*.app" -type d | head -n 1)
		if [ -z "$APP_BUNDLE" ]; then
			# Fallback to product.json name if no .app found
...
function code-wsl()
{
	...
	# in a wsl shell
	ELECTRON="$ROOT/.build/electron/Ritivel.exe"
	if [ -f "$ELECTRON"  ]; then
		local CWD=$(pwd)
		cd $ROOT
		export WSLENV=ELECTRON_RUN_AS_NODE/w:VSCODE_DEV/w:$WSLENV
		local WSL_EXT_ID="ms-vscode-remote.remote-wsl"
		local WSL_EXT_WLOC=$(echo "" | VSCODE_DEV=1 ELECTRON_RUN_AS_NODE=1 "$ROOT/.build/electron/Ritivel.exe" "out/cli.js" --locate-extension $WSL_EXT_ID)
		cd $CWD
